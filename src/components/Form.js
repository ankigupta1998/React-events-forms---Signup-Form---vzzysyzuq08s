...
const Form = () => {
  ...
  const PassValidClass =
    isTouchedForPass && valueIsValidForPass
      ...
      : "name-input";
  const OptValidClass =
  isTouchedForOpt&&valueIsValidForOpt?"name-input-invalid"
  : "name-input";
    ...
    <>
      <div className="container">
        <form onSubmit={submitHandler} >
          ...
          <select className={OptValidClass} data-testid="gender" id="gender" defaultValue="Male" onBlur={inputblurHandlerForOpt} onClick={valueChangeHandlerForopt} >
            <option>Male</option>
            <option>Female</option>
            <option>others</option>
          </select>
          <br />
          {isTouchedForOpt && valueIsValidForOpt &&<div>Please identify as male, female or others</div>}
          ...
          <button type="submit" data-testid="submit" onClick={()=>{
            !valueIsValidForPass && !valueIsValidForNo && !valueIsValidForOpt && !valueIsValidForEmail && !valueIsValid && alert ( `Hello ${result} `)
          }} >
            Submit
          </button>
            </div>
